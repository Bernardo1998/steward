...
def generate_promoted_workflow(data: dict, analysis: dict) -> dict:
    ...
    if task_pattern == "cycle":
        target_desc = """Generate a CycleRunner subclass. Import from steward.runner:
...
- The script must write summary.json and summary.md to the summary directory
  (read from STEWARD_SUMMARY_DIR env var or construct from STEWARD_INSTANCE_ROOT)
...
- Use call_llm_json() from steward.llm for any step needing LLM
...
def main():
    ...
    instance_root = Path(args.instance_dir) if args.instance_dir else Path(
        os.environ.get("STEWARD_INSTANCE_ROOT", ".")
    )
