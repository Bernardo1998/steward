...
def _init_paths(instance_root: Path):
    ...
    # Also set env var so steward modules can find the instance root
    os.environ["STEWARD_INSTANCE_ROOT"] = str(REPO_ROOT)
...
def check_preflight(task_id: str, charter: dict) -> list[str]:
    ...
    try:
        from steward.preflight import check_constraints
...
2. If the fix requires a code or config change, MAKE THE CHANGE directly.
   ...
   - You can also read/edit files in the steward library at /mnt/c/steward/
...
Be concise. Focus on fixing, not explaining."""
    ...
    try:
        from steward.llm import call_agent_write
    ...
    try:
        from steward.reflection.state import append_diagnosis_history
...
def spawn_agent(task: dict, charter: dict, date_str: str) -> subprocess.Popen:
    ...
    if agent == "codex":
        ...
        if auto_resume:
            ...
            from steward.llm import build_agent_cmd
        ...
        env = os.environ.copy()
        env["STEWARD_INSTANCE_ROOT"] = str(REPO_ROOT)
        env["STEWARD_RUN_DATE"] = date_str
        env["STEWARD_SUMMARY_DIR"] = summary_dir
