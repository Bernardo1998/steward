...
Usage (minimal — just definition.yaml):
    python -m charter_worker.runner --definition definition.yaml --state state/
...
class CycleRunner:
    ...
    def __init__(
        ...
        if summary_dir:
            ...
            env_dir = os.environ.get("CHARTER_SUMMARY_DIR")
            if env_dir:
                self.summary_dir = Path(env_dir)
            else:
                date_str = datetime.now().strftime("%Y-%m-%d")
                instance_root = os.environ.get("CHARTER_INSTANCE_ROOT", ".")
                self.summary_dir = (
