...
def check_preflight(task_id: str, charter: dict) -> list[str]:
    ...
    try:
        from charter_worker.preflight import check_constraints
...
Be concise. Focus on fixing, not explaining."""
    ...
    try:
        from charter_worker.llm import call_agent_write
    ...
    try:
        from charter_worker.reflection.state import append_diagnosis_history
...
def spawn_agent(task: dict, charter: dict, date_str: str) -> subprocess.Popen:
    ...
    if agent == "codex":
        ...
        if auto_resume:
            ...
            from charter_worker.llm import build_agent_cmd
...
def main():
    ...
    if not reflection_ran and now.hour >= REFLECTION_HOUR:
        ...
        try:
            from charter_worker.reflection.pipeline import run_reflection
