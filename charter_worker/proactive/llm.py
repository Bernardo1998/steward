...
2. CHARTER_LLM_CLI env var (e.g. "codex", "claude", "my-custom-cli")
3. Auto-detect: first available on PATH from [codex, claude]

For custom CLIs, set CHARTER_LLM_CLI to the binary name and
CHARTER_LLM_CMD_TEMPLATE to a JSON config (see build_agent_cmd docs).
"""
...
def detect_provider() -> str:
    ...
    Checks CHARTER_LLM_CLI env var first, then auto-detects.
    """
    env_cli = os.environ.get("CHARTER_LLM_CLI", "").strip()
    if env_cli:
    ...
    raise RuntimeError(
        ...
        "or set CHARTER_LLM_CLI to your custom CLI binary."
    )


def _load_custom_template() -> Optional[dict]:
    """Load custom CLI template from CHARTER_LLM_CMD_TEMPLATE env var.
    ...
    """
    raw = os.environ.get("CHARTER_LLM_CMD_TEMPLATE", "").strip()
    if not raw:
        return None
    try:
    ...
    if custom and provider not in _PROVIDERS:
        ...
        if not template:
            raise ValueError(
                ...
                f"Set CHARTER_LLM_CMD_TEMPLATE with both 'read_only' and 'write' keys."
            )
        ...
        raise ValueError(
            ...
            f"For custom CLIs, set CHARTER_LLM_CMD_TEMPLATE env var."
        )
