...
from steward.actions import ActionResult, Action, WebSearchAction, LightweightSearchAction, ExperimentAction
...
class TestWebSearchAction:
    ...
    def test_execute_returns_action_result(self):
        ...
        with patch("steward.search.engine.run_research", return_value=mock_result):
    ...
    def test_execute_handles_failure(self):
        ...
        with patch("steward.search.engine.run_research", side_effect=RuntimeError("search failed")):
            result = ws.execute(action, context={})
    ...
    def test_execute_with_timeout(self):
        ...
        with patch("steward.search.engine.run_research", return_value={"synthesis": {}, "sources": []}):
            result = ws.execute(action, context={})
            assert result.status == "success"
...
class TestLightweightSearchAction:
    ...
    def test_execute_returns_action_result(self):
        ...
        with patch("steward.llm.call_llm", return_value=mock_output):
    ...
    def test_execute_handles_failure(self):
        ...
        with patch("steward.llm.call_llm", side_effect=RuntimeError("timeout")):
            result = ls.execute(action, context={})
...
class TestExperimentAction:
    ...
    def test_planning_failure(self, tmp_path):
        ...
        with patch("steward.llm.call_llm_json", side_effect=RuntimeError("LLM down")):
            result = ea.execute(action, context)
